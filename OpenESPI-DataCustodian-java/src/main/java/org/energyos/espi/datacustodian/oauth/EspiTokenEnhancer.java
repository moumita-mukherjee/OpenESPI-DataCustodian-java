...
public class EspiTokenEnhancer implements TokenEnhancer {
	...
	public OAuth2AccessToken enhance(OAuth2AccessToken accessToken,
		...
		result.setAdditionalInformation(Collections.singletonMap("client_id",
				(Object) authentication.getOAuth2Request().getClientId()));

		System.out.println("authentication.isClientOnly()  "+authentication.isClientOnly() );
		// TODO: Change subscription and authorization table entry creation to
		// work for "client_credentials" access token request
		// Is this access token for an application (i.e. "client_credentials"
		// request?
		if (authentication.isClientOnly() == false) { // No, continue
			...
			if (oAuth2Request.getExtensions() != null
					&& oAuth2Request.getExtensions().containsKey("usagepoint")) {
				usagePointId = (Long) oAuth2Request.getExtensions().get(
						"usagepoint");
			...
			Subscription subscription = subscriptionService
					.createSubscription(authentication,usagePointId);
			...
			RetailCustomer retailCustomer = ((User) authentication
					.getPrincipal()).getRetailCustomer();

			
			System.out.println("Linking with usagepoint----@ESPITokenEnhancer"
					+ usagePointId);
			if (usagePointId > 0) {

				UsagePoint up = resourceService.findById(usagePointId,
						UsagePoint.class);
			...
			authorization.setAuthorizedPeriod(new DateTimeInterval(24*3600*1000*365l, System.currentTimeMillis()));
			...
			authorization.setResourceURI(baseURL
					+ Routes.BATCH_SUBSCRIPTION.replace("{subscriptionId}",
