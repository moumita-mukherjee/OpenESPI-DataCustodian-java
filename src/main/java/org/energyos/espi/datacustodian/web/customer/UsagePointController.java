...
public class UsagePointController extends BaseController {
	...
	public String show(@PathVariable Long retailCustomerId, @PathVariable Long usagePointId, ModelMap model,
		...
		try {
			...
			HashMap<String, Object> displayBag = buildDisplayBag(currentUser(principal).getCustomerId(),
					currentCustomer(principal).getId(), usagePointId);
	...
	private HashMap<String, Object> buildDisplayBag(String customerId, Long retailCustomerId, Long usagePointId) {
		...
		while (it.hasNext()) {
			...
			mrBag.put("intervalReadingTill", mr.getIntervalReadingTill());
