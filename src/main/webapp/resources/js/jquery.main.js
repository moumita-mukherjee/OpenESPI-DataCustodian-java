...
function initHolderFixed() {
	jQuery('.tabs-holder').each(function() {
		var tabHolder = jQuery(this);
		var tabHolderHeight = tabHolder.outerHeight();
		var win = jQuery(window);
		var tabs = jQuery('.tabs');
		var holder = tabHolder.next('section');
		var tabHolderOffset = tabHolder.offset().top;
		var scrollPage = win.scrollTop();
		win.on('scroll', function() {
			scrollPage = win.scrollTop();
			classHandler();
		});
		function classHandler(){
			...
			if (scrollPage >= tabHolderOffset) {
				tabHolder.addClass('fixed');
				holder.css('marginTop',tabHolderHeight);
			} else {
				tabHolder.removeClass('fixed');
				holder.css('marginTop', '');
			}
		}
		classHandler();

		var blocks = jQuery();
		var listItems = jQuery('.tabs li');
		...
		listItems.each(function() {
			...
			if (block.length) {
				block.data('listItem', listItem);
				blocks = blocks.add(block);
			}
		});
		
		win.on('scroll resize orientationchange', pageScroll);
		
		function pageScroll() {
			var activeBlock;
			blocks.each(function() {
				var block = jQuery(this);
				if (block.offset().top-tabHolderHeight-150 < win.scrollTop()) {
					activeBlock = block;
				}
			});
			if (activeBlock) {
				if(typeof activeBlock.data('listItem') != 'undefined'){
					if (!activeBlock.data('listItem').hasClass('active')) {
						listItems.removeClass('active');
						activeBlock.data('listItem').addClass('active');
					}
				}
			}
		}
...
var loginAreaActive=function(){};
var loginAreaInActive=function(){};
//initMessage
function initMessage() {
	var msgBox = jQuery('#note');
	var msgData = false;
	var templateData = false;
	
	jQuery.getJSON(msgBox.attr('data-message'), function(data) {
		if (data) {
			msgData = data;
			createMessage();
		}
	...
	jQuery.get(msgBox.attr('data-template'), function(data) {
		templateData = data;
		createMessage();
	});
	
	function createMessage() {
		if (msgData && templateData) {
			msgBox.html(tmpl(templateData, msgData));
		}
	}
}
...
// refresh custom scrolls on resize
...
// popups init
function initPopups() {
	lib.each(lib.queryElementsBySelector('.popup .form .row2'), function(){
		new ContentPopup({
			holder: this,
			popup: '.popup-box',
			btnOpen: '.link-info',
...
function initSmoothScroll() {
	...
	var tabHolder = jQuery('.tabs-holder');
	if( typeof(tabHolder.offset()) !== 'undefined' ){
		var tabHolderHeights = tabHolder.outerHeight();
		var win = jQuery(window);
		var tabHolderOffset = tabHolder.offset().top;
		var scrollPage = win.scrollTop();
		...
		var isOldDevice = /(iPhone|iPad).*OS 5/.test(navigator.userAgent), fixTimer;
		var fixIOS5 = function() {
			if(!isOldDevice) return;
			clearTimeout(fixTimer);
			fixTimer = setTimeout(function() {
				var win = jQuery(window);
				var box = jQuery('.tabs-holder.fixed');
		...
		jQuery(document).on('click', 'a.to-scroll', function(e) {
			var target = jQuery(this.hash);
			if (target.length) {
				e.preventDefault();
				scrollElement.stop().animate({
					scrollTop: target.offset().top - tabHolderHeights
				}, {
					duration: animSpeed,
					complete: fixIOS5
				});
...
//Dont loose focus during transition flag
var currentBtnSlide = false;
...
function initCarouselFade(){
	...
	jQuery('.slideset2').bxSlider({
		auto: true,
		mode: 'fade',
		speed: 1000,
		pause: 8000,
		nextSelector:'#container-next',
		prevSelector:'#container-prev', 
		pagerSelector:'#pagination'
	});
	
	//Dont loose focus during transition code
	jQuery(document).keyup(function(e){
	if( jQuery('.slideset').find('a:focus').length > 0 ){
		currentBtnSlide = true;
	}else{
		currentBtnSlide = false;
	}
});
...
function initOpenCloseFooter() {
	...
	//jQuery('form.reset-form input[type="submit"]').on('click', function(e) {
	//	e.preventDefault();
	//	jQuery('span.error-required').remove();
		
	//	var field = jQuery('form.reset-form input#email-address');

		//validate
	//	if (field.val() == "") {
	//		field.after(createValidationMessage("required", field));
	//	}
...
// set active section in side-menu, if any, based on location hash value
function setActiveMenuItem() {
	// deactivate all items first, so at most one section is expanded at a time
	jQuery('.add-menu li').removeClass('active');

	// find link matching current page
	var link = jQuery('.add-menu a[href$="' + location.hash + '"]');
...
function initAccordion() {
	...
	jQuery('.add-menu ul').slideAccordion({
		opener: 'a.opener',
		slider: 'div.slide',
		animSpeed: 300
	});
	jQuery('.faq-list li').slideAccordion({
		opener: '.opener',
		slider: 'div.slide',
		animSpeed: 300
	});
...
function initSameHeight() {
	...
	jQuery('.main-alert').sameHeight({
		elements: '.alert-text, .alt-box2, .alt-text',
		multiLine:true,
		flexible: true
	});
}
...
function initDropDownClasses() {
	...
	jQuery('.landingnav').hover(function() {
		jQuery('li.account').removeClass('hover');
	});
}

// add classes to support css3 selectors in old browsers
function initChildClasses() {
	jQuery('.news-list ul').children(':nth-child(2n)').addClass('even');
	jQuery('.form .row').each(function(){
		jQuery(this).find('.area').last().addClass('last')
	})
}
...
function initAddClasses() {
	jQuery('.close').clickClass({
		classAdd: 'active',
		addToParent: 'personal-block'
	});
}

// add classes on hover/touch
function initCustomHover() {
	var navHolder = jQuery('#nav');
	var holders = navHolder.find('>li');
	var timer;
	holders.each(function(event) {
		var holder = jQuery(this);
		var classActive = 'hover';
		var isTouchDevice = ('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch;

		holder.on('keyup', function(e) {
			if (e.keyCode == 13) {
				clearTimeout(timer);
				if (holder.hasClass(classActive))
					holders.removeClass(classActive);
				else
					holder.addClass(classActive);
			}
		});

		holder.on('mouseenter touchstart',  function() {
			clearTimeout(timer);
			holders.removeClass(classActive);
            if (holder.attr("id") != "_login" && !holder.hasClass("has-drop-down")) {
                holder.addClass("has-drop-down");
            }
			if (!holder.hasClass(classActive)) {
				holder.addClass(classActive);
			}
		...
		if (isTouchDevice) {
			holder.on('touchleave touchend mouseenter', function(event) {
				if (jQuery('body').hasClass('homepage') && holder.hasClass('account')) {
			...
			holder.on('mouseleave', function(event) {
				if (jQuery('body').hasClass('homepage') && holder.hasClass('account')) {
				clearTimeout(timer);
					timer = setTimeout(function() {
						holder.removeClass(classActive);
		...
		jQuery(document).on('click touchstart', function(e) {
			...
			if (!target.parents().hasClass('nav-holder')) {
				holder.removeClass(classActive);
	...
	jQuery('.search-holder').touchHover();


	jQuery('#menu>li').on('keyup', function(e) {
		if (e.keyCode == 13) {
			var holder = jQuery(this);
			var classActive = "hover";
			if (holder.hasClass(classActive))
...
;(function($) {
	...
	Clock.prototype = {
		...
		findElements: function() {
			...
			this.timeFrame = this.holder.find(this.options.timeFrame);
			this.pd = this.holder.find(this.options.pd);
			this.usage = this.holder.find(this.options.usage);
			this.peakState = this.holder.find(this.options.peakState);
		},
		getData: function() {
			//
			// Changed by Affinity (Karim)
			//
			var that = this;
			var clockData = this.holder.scope().peakBarData;
			that.allData = clockData;
			if(!clockData)
				return;
			that.data = clockData.dailyData;
			var currentDate = new Date();
			var holidayServiceUrl = LHConfig.apiBaseUrl + '/v1/cms/holidays?year='+currentDate.getFullYear();
			var holidaysInAYear = getHolidaySchedule(holidayServiceUrl);
			that.holidaysInAYear = holidaysInAYear;
			var isWeekend = currentDate.getDay() == 6 || currentDate.getDay() == 0 ? true : false;
			if(isWeekend)
				that.data = clockData.weekendData;
			else if(isHoliday(holidaysInAYear, currentDate)) {
				that.data = clockData.holidayData;
			}
			if(that.data) {
				that.startClock();
			}
			/*$.getJSON(this.holder.attr(this.options.dataAttr), function(data) {
				that.data = data;
				if (that.data) {
					that.startClock();
				}
		...
		startClock: function() {
			var that = this;
			var currData = that.data;
			for(var di = 0; di < currData.length; di++) {
				var obj = currData[di];
				// an exception was being returned randomly.
				try {
					var timeLimit = '';
					var timeString = obj.splice(0, 1)[0].toString().replace(/,/g, '-');
					if(typeof timeString === 'string')
						timeLimit = timeString.split('-');
					else
						throw timeString;
					var limits = [];
					for (var dj = 0; dj < timeLimit.length; dj++) {
						if (timeLimit[dj].charAt(0) === '0') {
							timeLimit[dj] = timeLimit[dj].slice(-1);
						}
						timeLimit[dj] = parseInt(timeLimit[dj]);
						limits.push(timeLimit[dj]);
					}
					obj.splice(0, 0, limits);
				} catch(e) {
				}
			}
		...
		checkTime: function() {
			...
			that.timeFrame.text(((hours + '').length === 1 ? ('0' + hours) : hours) + ':' + ((minutes + '').length === 1 ? ('0' + minutes) : minutes));
			...
			for (var ct = 0; ct < that.data.length; ct++) {
				var obj = that.data[ct];
				if (hours >= obj[0][0] && hours < obj[0][1]) {
					// only 3 schemes to be used
					that.holder.removeClass('scheme1 scheme2 scheme3 scheme4 scheme5 scheme6');
					if(that.data[ct][2] == "off peak") {
						that.holder.addClass('scheme2');
					}
					else if(that.data[ct][2] == "mid peak") {
						that.holder.addClass('scheme5');
					}
					else {
						that.holder.addClass('scheme3');
					}
					that.usage.text(obj[3]);
					that.peakState.text(obj[2]);
					return false;
				}
			}
		}
	...
	$.fn.clock = function(options) {
		return this.each(function() {
			$(this).data('Clock', new Clock($.extend(options, {holder: this})));
		});
	}
}(jQuery));
...
;(function($){
	...
	ScrollGallery.prototype = {
		...
		switchSlide: function() {
			...
			this.tmpProps = {};
			this.tmpProps[this.animProperty] = this.getStepOffset();
			if( this.options.fadeSlide === false ){
				...
				var tmpProps = this.tmpProps
				this.slider.stop().fadeOut( this.options.fadeTime, function(){
					jQuery(this).animate(tmpProps, {duration: animSpeed, complete: function(){
						// animation complete
						self.galleryAnimating = false;
						if(self.resizeQueue) {
							self.onWindowResize();
...
;(function($){
	$.fn.slideAccordion = function(opt){
		...
		var options = $.extend({
			...
			animSpeed: 300,
			collapsible:true,
			event:'click.slideAccordion' // added namespace to event so we remove and rebind only exactly this handler
		},opt);
		...
		return this.each(function(){
			...
			items.each(function(){
				...
				opener.bind(options.event, function(e){
					if (options.event.length > 0 && options.event.indexOf('key') > -1) {
						if (e.keyCode != 13)
							return;
					}
...
jQuery.fn.clickClass = function(opt) {
	...
	return this.each(function() {
		...
		//
		// To avoid over-registering the event, we use a special namespace and
		// unbind any previous instances of the event before binding the one in the call.
		//

		var ev = options.event + '._clickClass';

		jQuery(this).off(ev);
		jQuery(this).on(ev, function(e) {
	...
	PlaceholderInput.prototype = {
		...
		togglePlaceholderText: function(state) {
			if(!this.element.readOnly && !this.options.focusOnly) {
				if(this.wrapWithElement) {
					if(this.fakeElement != undefined) //DO NOT TOUCH - AM
					{
...
jcf.addModule({
	...
	setupWrapper: function(){
		...
		this.fakeElement.style.width = (this.realElement.offsetWidth > 0 ? this.realElement.offsetWidth*1.2 + 'px' : 'auto');
